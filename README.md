...
- For header-only libraries: see the comments in [CMakeLists.txt](library/CMakeLists.txt)
...
Yes, however you will need to change the library type to an `INTERFACE` library as documented in the [CMakeLists.txt](library/CMakeLists.txt).
...
Instead, create a new directory or project with a CMakeLists that adds the library as a dependency (e.g. like the [standalone](CMakeLists.txt) directory).
